...
```mermaid
flowchart LR
    A[Registro o acceso como visitante] --> B[Selección de nivel]
    B --> C[Lección interactiva]
    C --> D[Ejercicio práctico]
    D --> E{Respuesta correcta}
    E -- Sí --> F[Gana XP y avanza]
    E -- No --> G[Recibe retroalimentación]
    G --> D
    F --> H[Desbloquea retos y medallas]
    H --> I[Continúa en el mapa de aprendizaje]
```
...
flowchart TB
    ...
    SERVICES --> REPO[Repositorios y CRUD]
    REPO --> MYSQL[(MySQL)]
    REPO --> MONGO[(MongoDB)]
    SERVICES --> MEDIA[Contenido multimedia: audios, imágenes y textos]
```
